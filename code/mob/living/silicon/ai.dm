...
/mob/living/silicon/ai/show_message(msg, type, alt, alt_type, group = 0)
	..()
	...
	if (actions.len > 1)
		...
		switch(user.a_intent)
			if(INTENT_HELP)
				if (isdead(src))
					if (src.health < 0)
						user.visible_message("<span style=\"color:red\"><b>[user.name]</b> pokes the restart button on  [src.name], but [src.name] is too damaged to power on.</span>")
					else
						user.visible_message("<span style=\"color:red\"><b>[user.name]</b> pokes the restart button on [src.name]...</span>")
						setalive(src)
...
/mob/living/silicon/ai/Life(datum/controller/process/mobs/parent)
	...
	if (isalive(src) && src.death_timer <= 0)
		src.show_text("<b>ALERT:</b> System power 99.9999% depleted. Initiating total core shutdown to prevent permanent system damage.", "red")
		src.death()
		return
	...
	if (src.health < 0)
		src.setStatus("paralysis", 1 SECONDS)
		src.death_timer--
	else
...
/mob/living/silicon/ai/proc/update_appearance()
	...
	if (isdead(src))
		...
		ClearAllOverlays()
	else if (src.power_mode == -1 || src.health < 0 || !src.brain || src.getStatusDuration("paralysis"))
