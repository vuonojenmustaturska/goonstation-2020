		...
		if("notes2")
			...
			switch(href_list["type"])
				...
				if("add")
					...
					var/the_note = input("Write your note here!", "Note for [player]") as null|text
	...
	//		var/list/alljobs = get_all_jobs()
		...
		if ("c_mode_current")
			if (src.level >= LEVEL_SA)
				...
				"spy","gang","disaster","changeling","vampire","mixed","mixed_rp", "construction","conspiracy","spy_theft","battle_royale", "vampire","assday")
