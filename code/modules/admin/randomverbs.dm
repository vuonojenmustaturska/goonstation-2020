...
/client/proc/cmd_admin_subtle_message(mob/M as mob in world)
	...
	if (usr && usr.client && usr.client.holder)
		boutput(M, __blue("You hear a voice in your head... <i>[msg]</i>"))
...
/client/proc/cmd_admin_pm(mob/M as mob in world)
	...
	do_admin_pm(M.ckey, usr) //Changed to work off of ckeys instead of mobs.
...
/datum/polymorph_menu
	...
	proc/copy_to_target()
		...
		if(src.update_wearid && target_mob.wear_id)
			target_mob.choose_name(1,1,target_mob.real_name)
...
/client/proc/cmd_admin_check_reagents(var/atom/target as null|mob|obj|turf in world)
	...
	<b>Temperature:</b> [reagents.total_temperature] K
	...
	boutput(usr, "<span style=\"color:blue\"><b>[target]'s contents: <i>[final_log]</i>. Temp: <i>[reagents.total_temperature] K</i></b></span>") // Added temperature (Convair880).
	usr.Browse(final_report, "window=reagent_report")

	logTheThing("admin", usr, null, "checked the reagents of [target] <i>(<b>Contents:</b>[final_log])</i>. <b>Temp:</b> <i>[reagents.total_temperature] K</i>) [log_loc(target)]")
	logTheThing("diary", usr, null, "checked the reagents of [target] <i>(<b>Contents:</b>[final_log])</i>. <b>Temp:</b> <i>[reagents.total_temperature] K</i>) [log_loc(target)]", "admin")
	return
